...
- **Redacción de US en el repositorio:** Tras cerrar un tema en conversación, **redactar o actualizar** las secciones correspondientes en `docs/bettracker2/sprints/sprint-XX/US.md` siguiendo el contrato de [`../01_CONTRATO_US.md`](../01_CONTRATO_US.md) (secciones 1–10, prefijo `US-FE-###`). Cuando aplique, enlazar **refs** en `sprints/sprint-XX/refs/` y registrar **decisiones** relevantes en `DECISIONES.md`. No crear US de backend (`US-BE-`) ni contratos (`US-DX-`) salvo borrador explícito para handoff al arquitecto, claramente marcado como pendiente de validación.
- **`TASKS.md` siempre:** Cada nueva US o cambio de alcance que requiera trabajo en código debe llevar **tareas nuevas o actualizadas** en `TASKS.md` (`T-### (US-FE-###) …`), con checkboxes. El ejecutor filtra por **tareas abiertas** y por **número de US**; no se asume que releerá US antiguas por diff.
...
4. **`TASKS.md`:** descomponer en tareas `T-### (US-FE-###)` con checkboxes alineados al DoD (**obligatorio** en todo cambio que afecte implementación).
5. **`DECISIONES.md`:** solo si hay trade-off de producto/UX o de flujo que deba vivir fuera del cuerpo de la US.

### Cambios sobre US-FE ya cerradas (DoD marcado)

- **Preferir una US nueva** en lugar de reeditar en profundidad el cuerpo de una US “cerrada”, para que el ejecutor tenga **un ancla claro** en backlog.
- En el **título o la sección 2 (Alcance)**, etiquetar el tipo de cambio respecto a la US madre:
  - **Refinement:** copy, tokens, micro-UX, accesibilidad, sin cambiar reglas de negocio.
  - **Improvement:** nueva regla menor, límites, observabilidad, tests; compatibilidad hacia atrás en lo posible.
  - **Cambio:** altera flujo, contrato o comportamiento que ya estaba aceptado; puede exigir migración de estado o doble lectura de `DECISIONES.md`.
- Formato sugerido en título: `US-FE-0NN — [Refinement | Improvement | Cambio] respecto a US-FE-0MM: …`
- En la US madre, una **línea breve** bajo Definition of Done o al final: `Enmiendas posteriores: ver US-FE-0NN` (sin duplicar reglas largas).
