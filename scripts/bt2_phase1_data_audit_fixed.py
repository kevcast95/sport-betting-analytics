...
def block_02_pending_lag(cur: Any, date_from: str, date_to: str, outdir: Path, sample_limit: int) -> BlockResult:
      ...
      SELECT
        *,
        ...
        CASE
          ...
          WHEN NOT (LOWER(COALESCE(status, '')) = ANY(%s)) AND (result_home IS NULL OR result_away IS NULL) THEN true
          ELSE false
    ...
    WHERE (
      ...
      OR (NOT (LOWER(COALESCE(status, '')) = ANY(%s)) AND (result_home IS NULL OR result_away IS NULL))
    )
