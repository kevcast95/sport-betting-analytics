...
def bt2_sm_strip_include_root(include_str: str, root: str) -> str:
    ...
    for p in parts:
        pl = p.lower()
        if pl == root_l or pl.startswith(root_l + "."):
            continue
        filtered.append(p)
...
def bt2_sm_next_include_on_forbidden(
    ...
    *,
    ...
    - Si el cuerpo indica un include prohibido y está en la cadena → cadena sin ese
      segmento (y sin anidados `root.*`), aunque `current` sea igual al ``core``
      (p. ej. ``inplayOdds`` encima del núcleo documentado).
    - Si no hay más segmentos que quitar y coincide con ``core`` → None.
    - Si no se puede parsear el include prohibido y aún no estamos en ``core`` → ``core``.
    """
